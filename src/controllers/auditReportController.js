...
function drawTable(doc, rows, columns) {
  ...
  rows.forEach((row, i) => {
    ...
    // Real, subtle alternating row shading - genuinely helps the eye
    // track a row across many columns on a dense financial table,
    // rather than every row reading as one undifferentiated block.
    if (i % 2 === 1) {
      doc.rect(startX, y - 2, 483, 15).fillColor('#f7f5f2').fill();
      doc.fillColor('#222');
    }
    x = startX;
    columns.forEach((c) => {
      doc.text(String(c.value(row)), x, y, { width: c.numeric ? c.width - 8 : c.width, align: c.numeric ? 'right' : 'left' });
      x += c.width;
    });
    y += 15;
  });
  // Real fix: reset x, not just y, so any .text() call right after this
  // table (with no explicit position of its own) starts at the left
  // margin instead of continuing from wherever the last column ended.
  doc.x = startX;
  doc.y = y + 4;
}

// A bordered, shaded callout for a key figure - genuinely distinct from
// a plain text line, and explicitly positioned at the left margin every
// time, so it can never inherit a stray cursor position left over from
// whatever was drawn before it.
function drawSummaryBox(doc, lines) {
  const startX = 56;
  const width = 483;
  const lineHeight = 18;
  const padding = 10;
  ...
  const boxY = doc.y;
  doc.rect(startX, boxY, width, boxHeight).fillColor('#f7f5f2').fill();
  doc.rect(startX, boxY, 3, boxHeight).fillColor(BRASS).fill();
  let textY = boxY + padding;
  lines.forEach((line) => {
    doc.fontSize(line.size || 11).font(line.bold ? 'Helvetica-Bold' : 'Helvetica').fillColor(line.color || INK)
      .text(line.text, startX + padding + 4, textY, { width: width - padding * 2 - 4 });
    textY += lineHeight;
  });
  doc.x = startX;
...
const generateBusinessAuditReport = asyncHandler(async (req, res) => {
  ...
  if (contracts?.length) {
    drawTable(doc, contracts, [
      ...
      { label: 'Annual total (AED)', value: (r) => Number(r.annual_total_aed).toFixed(2), width: 100, numeric: true },
  ...
  if (receipts?.length) {
    drawTable(doc, receipts, [
      ...
      { label: 'Amount (AED)', value: (r) => Number(r.amount).toFixed(2), width: 90, numeric: true },
  ...
  drawSummaryBox(doc, [{ text: `Subtotal: AED ${receiptsTotal.toFixed(2)}`, bold: true }]);
  ...
  if (payments?.length) {
    ...
    drawTable(doc, payments, [
      ...
      { label: 'Amount (AED)', value: (r) => Number(r.amount).toFixed(2), width: 90, numeric: true },
      { label: 'Tip (AED)', value: (r) => Number(r.tip_amount || 0).toFixed(2), width: 80, numeric: true },
      { label: 'Refunded', value: (r) => (r.refunded ? `Yes (${Number(r.refund_amount || 0).toFixed(2)})` : 'No'), width: 123 },
    ]);
    drawSummaryBox(doc, [{ text: `Subtotal: AED ${paymentsTotal.toFixed(2)}`, bold: true }]);
  }
  ...
  if (gatewayTxns?.length) {
    ...
    drawTable(doc, gatewayTxns, [
      ...
      { label: 'Amount (AED)', value: (r) => Number(r.amount_aed).toFixed(2), width: 123, numeric: true },
    ]);
    drawSummaryBox(doc, [{ text: `Charges: AED ${gatewayTotal.toFixed(2)}   ·   Refunds: AED ${gatewayRefunds.toFixed(2)}`, bold: true }]);
  }
  ...
  // empty trailing page. ~110pt is comfortably enough for the summary
  // box and the 3-sentence disclaimer at this font size.
  const summaryBlockHeight = 110;
  ...
  drawSummaryBox(doc, [{ text: `Total customer revenue for ${year}: AED ${grandTotal.toFixed(2)}`, bold: true, size: 13 }]);
...
const generatePlatformAuditReport = asyncHandler(async (req, res) => {
  ...
  const receiptsTotal = (receipts || []).reduce((s, r) => s + Number(r.amount), 0);
  const paidReceipts = (receipts || []).filter((r) => r.payment_status === 'paid');
  const unpaidReceipts = (receipts || []).filter((r) => r.payment_status !== 'paid');
  const paidTotal = paidReceipts.reduce((s, r) => s + Number(r.amount), 0);
  const outstandingTotal = unpaidReceipts.reduce((s, r) => s + Number(r.amount), 0);
  const signedClientCount = new Set((contracts || []).map((c) => c.business_id)).size;

  // Real overview, up front - every key figure at a glance before the
  // detailed tables, not something you only find by reading to the
  // bottom of the last page.
  drawSectionTitle(doc, 'Overview');
  drawSummaryBox(doc, [
    { text: `Signed clients: ${signedClientCount}`, bold: true },
    { text: `Combined contracted annual value: AED ${contractsAnnualTotal.toFixed(2)}` },
    { text: `Total billed in ${year}: AED ${receiptsTotal.toFixed(2)}` },
    { text: `Collected: AED ${paidTotal.toFixed(2)}`, color: '#166534' },
    { text: `Outstanding: AED ${outstandingTotal.toFixed(2)}`, color: outstandingTotal > 0 ? '#b91c1c' : INK },
  ]);
  ...
  if (contracts?.length) {
    drawTable(doc, contracts, [
      ...
      { label: 'Contract No.', value: (r) => r.contract_number, width: 110 },
      { label: 'Annual total (AED)', value: (r) => Number(r.annual_total_aed).toFixed(2), width: 110, numeric: true },
      { label: 'Status', value: (r) => r.status, width: 73 },
    ]);
  } else {
    doc.fontSize(10).fillColor('#888').font('Helvetica').text('No signed agreements on file.');
  ...
  drawSummaryBox(doc, [
    { text: `Combined contracted annual value: AED ${contractsAnnualTotal.toFixed(2)}`, bold: true },
  ]);
  ...
  if (receipts?.length) {
    drawTable(doc, receipts, [
      ...
      { label: 'Amount (AED)', value: (r) => Number(r.amount).toFixed(2), width: 85, numeric: true },
  ...
  // Real, new visibility this report never had: which specific clients
  // still owe money, not just a lump "of which collected" figure with
  // no way to see who the outstanding portion actually belongs to.
  if (unpaidReceipts.length) {
    drawSectionTitle(doc, 'Outstanding Receipts');
    drawTable(doc, unpaidReceipts, [
      { label: 'Client', value: (r) => r.businesses?.name || '—', width: 160 },
      { label: 'Receipt No.', value: (r) => r.receipt_number, width: 110 },
      { label: 'Date', value: (r) => new Date(r.created_at).toLocaleDateString('en-GB'), width: 90 },
      { label: 'Amount (AED)', value: (r) => Number(r.amount).toFixed(2), width: 100, numeric: true },
    ]);
  }
  ...
  drawSummaryBox(doc, [
    { text: `Total billed to clients in ${year}: AED ${receiptsTotal.toFixed(2)}`, bold: true, size: 13 },
    { text: `Collected: AED ${paidTotal.toFixed(2)}`, color: '#166534' },
    { text: `Outstanding: AED ${outstandingTotal.toFixed(2)}`, color: outstandingTotal > 0 ? '#b91c1c' : INK },
  ]);
  doc.fontSize(9).font('Helvetica').fillColor('#888').text(
